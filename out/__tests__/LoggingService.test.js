...
describe('LoggingService', () => {
    ...
    beforeEach(() => {
        ...
        loggingService = new LoggingService_1.LoggingService(mockOutputChannel, LoggingService_1.LogLevel.DEBUG);
    });
    afterEach(() => {
        jest.clearAllMocks();
    });
    describe('Log Level Management', () => {
        it('should set log level correctly', () => {
            loggingService.setLogLevel(LoggingService_1.LogLevel.ERROR);
    ...
    describe('Error Logging', () => {
        it('should log error with context', () => {
            ...
            expect(mockOutputChannel.appendLine).toHaveBeenCalledWith(expect.stringMatching(/\[ERROR\].*\[TestService\].*Operation failed: Test error/));
        });
        it('should log error without Error object', () => {
            loggingService.error('TestService', 'Simple error message');
            expect(mockOutputChannel.appendLine).toHaveBeenCalledWith(expect.stringMatching(/\[ERROR\].*\[TestService\].*Simple error message/));
        });
    ...
    describe('Warning Logging', () => {
        it('should log warning message', () => {
            ...
            expect(mockOutputChannel.appendLine).toHaveBeenCalledWith(expect.stringMatching(/\[WARN\].*\[TestService\].*Warning message/));
        });
    ...
    describe('Info Logging', () => {
        it('should log info message', () => {
            ...
            expect(mockOutputChannel.appendLine).toHaveBeenCalledWith(expect.stringMatching(/\[INFO\].*\[TestService\].*Info message/));
        });
    });
    describe('Debug Logging', () => {
        it('should log debug message with data', () => {
            ...
            expect(mockOutputChannel.appendLine).toHaveBeenCalledWith(expect.stringMatching(/\[DEBUG\].*\[TestService\].*Debug message/));
            expect(mockOutputChannel.appendLine).toHaveBeenCalledWith(expect.stringContaining(JSON.stringify(testData, null, 2)));
        });
    });
    describe('Performance Logging', () => {
        it('should log performance metrics', () => {
            loggingService.performance('TestService', 'testOperation', 150);
            expect(mockOutputChannel.appendLine).toHaveBeenCalledWith(expect.stringMatching(/\[INFO\].*\[TestService\].*testOperation completed in 150ms/));
        });
