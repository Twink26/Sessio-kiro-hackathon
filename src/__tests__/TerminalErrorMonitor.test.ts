...
describe('TerminalErrorMonitor', () => {
  ...
  beforeEach(() => {
    ...
    (vscode.window.onDidOpenTerminal as jest.Mock).mockReturnValue(mockDisposable);
    (vscode.window.onDidCloseTerminal as jest.Mock).mockReturnValue(mockDisposable);
    (vscode.window as any).terminals = mockTerminals;

    // Create new instance
    terminalErrorMonitor = new TerminalErrorMonitor();
  });
  ...
  describe('initialization', () => {
    ...
    it('should attach to existing terminals', () => {
      ...
      const newMonitor = new TerminalErrorMonitor();
